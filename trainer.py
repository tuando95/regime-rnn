...
class Trainer:
    ...
    def train(self) -> nn.Module:
        ...
        # Main training loop
        # Wrap epoch loop with tqdm for progress tracking
        ...
        for epoch in epoch_pbar:
            ...
            for X_batch, y_batch, _ in batch_pbar:
                ...
                if hasattr(self.model, "init_hidden"):
                    h_prev = self.model.init_hidden(B, device=self.device)
                else:
                    # No hidden state initialization provided
                    h_prev = [torch.zeros(0)]  # dummy to satisfy unpacking
                ...
                if self.use_amp and self.scaler is not None:
                    with torch.amp.autocast(device_type='cuda', dtype=torch.float16, enabled=self.use_amp):
                        for t in range(T_seq):
                            x_t = X_batch[:, t, :]
                            y_t = y_batch[:, t, :] # Target is already aligned in the dataset
                            h_prev, y_hat, g_t = self.model(x_t, h_prev)

                            mse_t = F.mse_loss(y_hat, y_t, reduction="mean")
                            ent_t = -(g_t * torch.log(g_t + utils.EPS)
                                     ).sum(dim=1).mean()
                    ...
                    for t in range(T_seq):
                        x_t = X_batch[:, t, :]
                        y_t = y_batch[:, t, :] # Target is already aligned
                        h_prev, y_hat, g_t = self.model(x_t, h_prev)
            ...
            with torch.no_grad():
                for X_batch, y_batch, _ in self.val_loader:
                    ...
                    y_batch = y_batch.to(self.device)
                    B, T_seq, _ = X_batch.shape
                    # Loop over the entire sequence length T_seq for validation

                    # Re-init hidden
                    if hasattr(self.model, "init_hidden"):
                        h_prev = self.model.init_hidden(B, device=self.device)
                    else:
                        h_prev = [torch.zeros(0)]
                    ...
                    for t in range(T_seq):
                        ...
                        sum_mse_val = sum_mse_val + mse_t
                        sum_ent_val = sum_ent_val + ent_t

                    # normalize validation losses by number of steps
                    sum_mse_val = sum_mse_val / float(T_seq)
                    sum_ent_val = sum_ent_val / float(T_seq)

                    val_samples += B
                    val_mse_sum += sum_mse_val.item() * B
                    val_ent_sum += sum_ent_val.item() * B
            ...
            epoch_pbar.set_postfix({
                ...
                "best_val_mse": f"{best_val_mse:.4f}"
            })

            # === Optuna Pruning Callback ===
            if self.epoch_callback:
                self.epoch_callback(epoch, avg_val_mse)

            # === Early Stopping Check ===
            if avg_val_mse < best_val_mse - 1e-12:
                ...
                if epochs_no_improve >= self.early_stopping_patience:
                    final_epoch = epoch # Record early stopping epoch
                    break
